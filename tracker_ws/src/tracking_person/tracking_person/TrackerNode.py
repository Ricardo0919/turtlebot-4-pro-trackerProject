...
class TrackerNode(Node):
    def __init__(self):
        ...
        self.rgb_topic = self.declare_parameter('rgb_topic', '/oakd/rgb/preview/image_raw').get_parameter_value().string_value
        self.model_file = self.declare_parameter('model_file', 'TrackerPerson.pt').get_parameter_value().string_value
        self.conf_thres = self.declare_parameter('conf', 0.65).get_parameter_value().double_value

        # Entrenamiento 160x120
        self.target_w = self.declare_parameter('target_w', 160).get_parameter_value().integer_value
        self.target_h = self.declare_parameter('target_h', 120).get_parameter_value().integer_value
    ...
    def image_cb(self, msg: Image):
        ...
        best_box = None  # (x1,y1,x2,y2) en espacio 160x120
        ...
        try:
            ...
            if hasattr(r0, 'boxes') and r0.boxes is not None and len(r0.boxes):
                for b in r0.boxes:
                    ...
                    label = self.names[cls_id] if (isinstance(self.names, list) and 0 <= cls_id < len(self.names)) else str(cls_id)
                    if conf > top_conf:
                ...
                cv2.putText(annotated, 'no detections', (8, 24),
        ...
        # 5) Publicar imagen anotada (160x120)
        try:
